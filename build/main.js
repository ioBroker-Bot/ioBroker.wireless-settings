...
class NetworkSettings extends adapter_core_1.Adapter {
    ...
    async readConnectionProfile(connection, depth = 0) {
        // Guard against pathological master chains.
        if (depth > 4) {
            return { dhcp: false, ip4: '', ip4subnet: '', gateway: '', dns: [] };
        }
        const slaveType = NetworkSettings.firstNonEmptyLine(await this.getNmcliConnectionField(connection, 'connection.slave-type'));
        if (slaveType) {
            // Slave profiles have no ipv4 settings of their own — read the master's.
            const master = NetworkSettings.firstNonEmptyLine(await this.getNmcliConnectionField(connection, 'connection.master'));
            if (master) {
